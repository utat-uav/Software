...
#include "D:/Workspace/TensorflowBuild/tensorflow/tensorflow/contrib/cmake/build/tensorflow/cc/ops/sdca_ops.h"
...
SdcaOptimizer::SdcaOptimizer(const ::tensorflow::Scope& scope,
  ...
  for (int64 i = _outputs_range["out_delta_sparse_weights"].first; i < _outputs_range["out_delta_sparse_weights"].second; ++i)
    this->out_delta_sparse_weights.push_back(Output(ret, i));
  for (int64 i = _outputs_range["out_delta_dense_weights"].first; i < _outputs_range["out_delta_dense_weights"].second; ++i)
