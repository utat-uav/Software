...
#include "D:/Workspace/TensorflowBuild/tensorflow/tensorflow/contrib/cmake/build/tensorflow/cc/ops/ctc_ops.h"
...
CTCBeamSearchDecoder::CTCBeamSearchDecoder(const ::tensorflow::Scope& scope,
  ...
  for (int64 i = _outputs_range["decoded_indices"].first; i < _outputs_range["decoded_indices"].second; ++i)
    this->decoded_indices.push_back(Output(ret, i));
  for (int64 i = _outputs_range["decoded_values"].first; i < _outputs_range["decoded_values"].second; ++i)
    this->decoded_values.push_back(Output(ret, i));
  for (int64 i = _outputs_range["decoded_shape"].first; i < _outputs_range["decoded_shape"].second; ++i)
